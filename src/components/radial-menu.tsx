import { Link } from 'react-router-dom';

interface IRadialMenuProps {
    categories: string[] | null;
    currentCategory: string
}

export default function RadialMenu({ categories, currentCategory }: IRadialMenuProps) {

    if (!categories) return null;

    const countLinks = categories.length;
    const radius = 265;
    const centerX = 265;
    const centerY = 265;

    const angleOffset = - Math.PI / 3;

    const getPosition = (index: number) => {
        const angle = (2 * Math.PI * index) / countLinks + angleOffset;

        const x = centerX + radius * Math.cos(angle);
        const y = centerY + radius * Math.sin(angle);

        return { x, y };
    };

    return (
        <div className='circular-menu'>
            <ul className="circular-list">
                {
                    categories.map((category, index) => {
                        const { x, y } = getPosition(index);

                        return (
                            <li
                                key={category}
                                className='circular-item'
                                style={{ transform: `translate(${x}px, ${y}px) translate(-50%, -50%)` }}
                            >
                                <Link to={`/${category}`} className={`circular-item__link ${currentCategory === category ? 'circular-item__link-active' : ''}`}  >
                                    <span className='circular-item__link-text'>{category} </span>
                                    <p className='circular-item__link-index'>{index + 1}</p>
                                </Link>
                            </li>
                        )
                    })
                }
            </ul>
        </div>

    );
}